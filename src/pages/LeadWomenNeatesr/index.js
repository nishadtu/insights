...
const LeadWomenNearest = () => {
	const [users, loading, error] = useCollection(
		query(userCollectionRef, where('gender', '==', 'Female'), orderBy("score", 'asc'), orderBy("register_date", 'desc'),),
		{
	...
	useEffect(() => {
		...
		setTimeout(() => navigate(routeNames.leadNearestMen), 30000);
	}, [navigate]);
	...
	return (
		...
		<>
			...
			{loadingScreen && (
				<div id="leaderBoard">
					<HeaderLead text="Nearest the Pin | Women's Leaderboard" title="Leaderboard" titleStyle={{ display: 'none' }} />
					<Paper sx={paperBox}	>
...
export default LeadWomenNearest;
