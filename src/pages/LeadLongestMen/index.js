...
const LeadLongestMen = () => {
	const [users, loading, error] = useCollection(
		query(userCollectionRef, where('gender', '==', 'Male'), where('longest', '!=', ''), orderBy("longest", 'desc'), orderBy("register_date", 'desc'),),
		{
	...
	useEffect(() => {
		...
		setTimeout(() => navigate(routeNames.leadLongestWomen), 30000);
	}, [navigate]);
	...
	return (
		<>
			...
			{loadingScreen && (
				<div id="leaderBoard">
					<HeaderLead text="Longest Drive | Men's Leaderboard" title="Leaderboard" titleStyle={{ display: 'none' }} />
					<Paper sx={paperBox}	>
